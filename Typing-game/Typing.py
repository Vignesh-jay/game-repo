...
pygame.display.set_caption("Jay's - Keyboard Typing Game")
background = pygame.image.load('flash-bg.jpg')
...
def game_front_screen():
    ...
    if not game_over :
        ...
        draw_text(gameDisplay, "Press a key to begin !", 54, WIDTH / 1.5, 500)
    pygame.display.flip()
    waiting = True
    while waiting:
...
while True:
    ...
    character = pygame.image.load('jay.png')
    ...
    y_cor += word_speed
    gameDisplay.blit(wood,(x_cor-50,y_cor))
    gameDisplay.blit(character,(x_cor-150,y_cor))
    draw_text(gameDisplay, str(displayword), 40, x_cor-5, y_cor-5)
